  ...
  return (
    <div className='pt-2 bg-white flex flex-col rounded-xl'>
        ...
        <span className='text-md font-semibold text-gray-500 py-2 text-center'>Todays Timeline</span>
          ...
          <div className='w-full grid grid-cols-12 border-b-2 border-gray-600'>
                  ...
                  {timelineTasks.filter(habit => habit.timeofday.includes('05:')).map((timelinehabit) => (
                      <>
                          <div key={timelinehabit._id} className='flex flex-col items-center'>
                              <div className={`border-2 p-1 rounded-full ${categoryBorder[timelinehabit.category]} `}> {timelinehabit.icon} </div>
                          </div>
                      </>
                  ))}
                  <div className={`min-h-6 border-r-2 border-gray-600`}> </div>
              </div>
              ...
              <div className='flex flex-col justify-end items-center'>
                  {timelineTasks.filter(habit => habit.timeofday.includes('19:')).map((timelinehabit) => (
                      <>
                          <div key={timelinehabit._id} className='flex flex-col items-center'>
                              <div className={`border-2 p-1 rounded-full ${categoryBorder[timelinehabit.category]} `}> {timelinehabit.icon} </div>
                              <div className={`min-h-6 border-r-2 ${categoryBorder[timelinehabit.category]}`}> </div>
                          </div>
                      </>
                  ))}
                  <div className={`min-h-6 border-r-2 border-gray-600`}> </div>
